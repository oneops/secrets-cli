...
public class BriefLogFormatter extends Formatter {
  ...
  private final MessageFormat messageFormat =
      new MessageFormat("{4,date,HH:mm:ss} [" + Platform.getUser() + "] {0} {1}{2}.{3}: {5}\n{6}");

  /**
   * OpenJDK made a questionable, backwards incompatible change to the Logger implementation. It
   * internally uses weak references now which means simply fetching the logger and changing its
   * configuration won't work. We must keep a reference to our custom logger around.
   */
  ...
  /**
   * Configures JDK root logging to use this class for everything. This will remove the default
   * console logger and adds a custom file handler.
   */
  public static void init() throws IOException {
    ...
    if (!initialized) {
      ...
      FileHandler fileHandler =
          new FileHandler(CliConfig.logPath.toString(), 10 * 1024 * 1024, 2, true);
