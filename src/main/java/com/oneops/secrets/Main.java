            ...
            switch (err.getStatus()) {
                case 403: {
                    buf.append(lineSep)
                            ...
                            .append(lineSep)
                            .append("  ")
                            .append(yellow(dot(String.format("User '%s' is part of the organization '%s'.", getUser(), app.getOrg()))))
                            .append(lineSep)
                            .append("  ")
                            .append(yellow(dot(String.format("User '%s' is part of the 'secrets-admin' or the 'secrets-admin-%s' team in the organization.", getUser(), app.getAssembly().toLowerCase()))))
                            .append(lineSep)
                            .append("    ")
                            .append(yellow(String.format("Contact an administrator of the organization '%s' to create the team(s), if it doesn't exist.", app.getOrg())))
                            .append(lineSep)
                            .append("  ")
                            .append(yellow(dot(String.format("Secrets admin team that you are a part of is assigned to the assembly '%s'.", app.getAssembly()))))
                            .append(lineSep)
                            .append("    ")
