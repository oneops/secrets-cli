...
public class Secret {
  ...
  public static String getTable(List<Secret> secrets) {
    List<Column.Data<Secret>> columns =
        Arrays.asList(
  ...
  /**
   * Returns formatted table string for the given secret details. The table is inverted as there are
   * many columns. For 'Updated By' user, we are using a custom metadata attribute because of the
   * Keywhiz limitation.
   *
   * @return formatted string.
   */
  ...
  private static String expiry(long atMilli) {
    if (atMilli != 0) {
      return ZonedDateTime.ofInstant(Instant.ofEpochMilli(atMilli), ZoneId.systemDefault())
          .format(dateFormatter);
    } else {
      return "Never";
    }
