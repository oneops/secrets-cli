        ...
        try {
            ...
            if (result.isSuccessful()) {
                ...
                buf.append(sux(format("Secret '%s' added successfully for application %s.", secretName, app.getNsPath())))
                        ...
                        .append(lineSep)
                        .append("  ")
                        .append(yellow(dot(String.format("Secret '%s' will be synced to '%s' environment computes in few seconds.", secretName, app.getNsPath().toLowerCase()))))
                        .append(lineSep)
                        .append("  ")
                        .append(yellow(dot(String.format("Applications can access secret content by reading '/secrets/%s' file.", secretName))))
                        .append(lineSep)
                        .append("  ")
                        .append(yellow(dot(String.format("Secrets can be accessed only by the User/Group configured in 'secrets-client' component in the '%s' environment.", app.getNsPath().toLowerCase()))))
                        .append(lineSep)
                        .append("  ")
                        .append(yellow(dot("You may need to restart the application in order for the secret change to take effect.")))
                        .append(lineSep)
                        .append("  ")
                        .append(yellow(dot("For security reasons, secrets are never persisted on the disk and can only be accessed via the '/secrets' virtual memory file system.")))
                        .append(lineSep);
