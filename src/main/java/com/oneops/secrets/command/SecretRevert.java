...
public class SecretRevert extends SecretsCommand {
  ...
  @Option(
    name = "-i",
    title = "Version index",
    description = "Secret version index.",
    required = true
  )
  public long version;
    ...
    try {
      ...
      if (result.isSuccessful()) {
        ...
        buf.append(
                sux(
                    format(
                        "Reverted the secret '%s' version to '%d' for the application %s.",
                        secretName, version, app.getNsPath())))
            .append(lineSep)
            .append(lineSep)
            ...
            .append(
                yellow(
                    dot(
                        String.format(
                            "Secret '%s' version '%d' will be synced to '%s' environment computes in a few seconds.",
                            secretName, version, app.getNsPath().toLowerCase()))))
            .append(lineSep)
            .append("  ")
            .append(
                yellow(
