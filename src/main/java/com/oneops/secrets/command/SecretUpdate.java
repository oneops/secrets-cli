    ...
    try {
      ...
      if (result.isSuccessful()) {
        ...
        buf.append(
                sux(
                    format(
                        "Updated the secret '%s' for application %s.",
            ...
            .append(
                yellow(
                    dot(
                        String.format(
                            "Secret '%s' will be synced to '%s' environment computes in a few seconds.",
            ...
            .append(
                yellow(
                    dot(
                        String.format(
                            "Applications can access secret content by reading the '/secrets/%s' file.",
                            secretName))))
            .append(lineSep)
            .append("  ")
            .append(
                yellow(
                    dot(
                        String.format(
                            "Secrets can be accessed only by the User/Group configured in the 'secrets-client' component in the environment '%s'.",
            ...
            .append(
                yellow(
                    dot(
                        "You can revert back to a previous version of the secret. Check 'secrets help revert' for more details.")))
            .append(lineSep);
