        ...
        try {
            ...
            if (result.isSuccessful()) {
                ...
                buf.append(sux(format("Updated the secret '%s' for application %s.", secretName, app.getNsPath())))
                        ...
                        .append(lineSep)
                        .append("  ")
                        .append(yellow(dot(String.format("Secret '%s' will be synced to '%s' environment computes in a few seconds.", secretName, app.getNsPath().toLowerCase()))))
                        .append(lineSep)
                        .append("  ")
                        .append(yellow(dot(String.format("Applications can access secret content by reading the '/secrets/%s' file.", secretName))))
                        .append(lineSep)
                        .append("  ")
                        .append(yellow(dot(String.format("Secrets can be accessed only by the User/Group configured in the 'secrets-client' component in the environment '%s'.", app.getNsPath().toLowerCase()))))
                        .append(lineSep)
                        .append("  ")
