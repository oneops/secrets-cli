...
@Command(
  name = "versions",
  description = "Retrieve versions of a secret, sorted from newest to oldest update time."
)
    ...
    try {
      ...
      if (result.isSuccessful()) {
        ...
        println(
            sux(
                format(
                    "Retrieved %d version(s) of secret '%s' for the application environment: %s",
                    secrets.size(), secretName, app.getNsPath())));
        if (secrets.size() > 0) {
